...
CREATE DEFINER =`root`@`localhost` TRIGGER `update_total_unit_price`
    BEFORE INSERT
    ON `item_transaction`
    FOR EACH ROW
BEGIN
...
CREATE DEFINER =`root`@`localhost` TRIGGER `update_total_transaction_price`
    AFTER INSERT
    ON `item_transaction`
    FOR EACH ROW
BEGIN
...
CREATE DEFINER =`root`@`localhost` TRIGGER `update_total_transaction_price_on_update`
    AFTER UPDATE
    ON `item_transaction`
    FOR EACH ROW
BEGIN
...
CREATE DEFINER =`root`@`localhost` TRIGGER `update_total_transaction_price_on_delete`
    AFTER DELETE
    ON `item_transaction`
    FOR EACH ROW
BEGIN
